...
func setDistance(planet *OrbitData) {
	...
	logger.Info(
		"set distance",
		zap.String("planet", planet.name),
		zap.Int("distance", distance),
	)

	for _, orbitingPlanet := range planet.data {
		orbitingPlanet.distance = distance
		setDistance(orbitingPlanet)
	}
}

func orbitCount(planet *OrbitData) int {
	total := planet.distance
	logger.Info(
		"orbit count",
		zap.String("planet", planet.name),
		zap.Int("distance", total),
	)

	for _, orbitingPlanet := range planet.data {
		total += orbitCount(orbitingPlanet)
	}

	return total
}

func getPlanet(name string) *OrbitData {
	planet := orbitMap[name]
	if planet == nil {
		planet = &OrbitData{name: name}
		orbitMap[name] = planet
	}
