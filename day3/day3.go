...
func shortestManhattanDistance(intersections Points) int {
	...
	for _, intersection := range intersections {
		x := math.Abs(float64(intersection[0]))
		y := math.Abs(float64(intersection[1]))
...
func intersectPoints(w1Points, w2Points Points) Points {
	...
	for w1Index := 1; w1Index < len(w1Points); w1Index++ {
		...
		for w2Index := 1; w2Index < len(w2Points); w2Index++ {
			...
			if intersect, point := intersectionPoint(w1p1, w1p2, w2p1, w2p2); intersect {
...
func intersectionPoint(A, B, C, D Point) (bool, Point) {
	a1 := B[1] - A[1]
	b1 := A[0] - B[0]
	c1 := a1*A[0] + b1*A[1]
	...
	if determinant == 0 {
		return false, Point{int(math.Inf(1)), int(math.Inf(1))}
	}
	...
	if isBetween(A, B, Point{x, y}) && isBetween(C, D, Point{x, y}) {
		logger.Info(
			"intersection-point",
			zap.Int("x", x),
			zap.Int("y", y),
...
func calculatePoints(wire []string) Points {
	var coordinates = make(Points, len(wire)+1)

	coordinates[0] = Point{0, 0}
	curPos := coordinates[0]
	for index, movement := range wire {
		direction, size := dirAndSize(movement)
		switch direction {
		case "R":
			coordinates[index+1] = Point{curPos[0] + size, curPos[1]}
		case "L":
			coordinates[index+1] = Point{curPos[0] - size, curPos[1]}
		case "D":
			coordinates[index+1] = Point{curPos[0], curPos[1] - size}
		case "U":
			coordinates[index+1] = Point{curPos[0], curPos[1] + size}
		}
		curPos = coordinates[index+1]
	}

	logger.Sugar().Infof("coordinates: %+v", coordinates)
	return coordinates
}

func dirAndSize(movement string) (string, int) {
	direction := string(movement[0])
	size, _ := strconv.Atoi(movement[1:])
